...
var deferred = _interopDefault(require('@livechat/deferred'));
var redux = require('redux');
var createSideEffectsMiddleware = _interopDefault(require('@livechat/side-effects-middleware'));
...
var Backo = _interopDefault(require('backo2'));
var unfetch = _interopDefault(require('unfetch'));
var urlUtils = require('@livechat/url-utils');
var createPlatformClient = _interopDefault(require('@livechat/platform-client'));
var mitt = _interopDefault(require('@livechat/mitt'));
var rawUploadFile = _interopDefault(require('@livechat/file-upload'));
var createAuth = require('@livechat/customer-auth');
var createAuth__default = _interopDefault(createAuth);
...
function finalCreateStore(initialStateData) {
  ...
  var store = redux.createStore(createReducer$1(createInitialState(initialStateData)), compose(redux.applyMiddleware(sideEffectsMiddleware)));
...
var getSideStorage = function getSideStorage(store) {
  return storage.getItem(getSideStorageKey(store)).catch(dataUtils.noop).then(function (sideStorage) {
...
var saveSideStorage = function saveSideStorage(store, sideStorage) {
  return storage.setItem(getSideStorageKey(store), JSON.stringify(sideStorage)).catch(dataUtils.noop);
};
...
var getEventsSeenUpToMap = function getEventsSeenUpToMap(users) {
  return dataUtils.mapValues(function (user) {
    return user.events_seen_up_to ? user.events_seen_up_to : null;
  }, dataUtils.keyBy('id', users));
};
...
var parseListChatsResponse = function parseListChatsResponse(payload) {
  ...
  return {
    ...
    users: dataUtils.uniqBy(function (user) {
      return user.id;
    }, dataUtils.flatMap(function (summary) {
...
var log = function log(_ref) {
  ...
  return makeGraylogRequest('https://queue.livechatinc.com/logs', urlUtils.buildQueryString(body));
};
...
var init = function init(config, env) {
  ...
  var auth = createAuth__default(instanceConfig, env);
  ...
  var api = Object.freeze({
    ...
    updateCustomerPage: function updateCustomerPage(page) {
      store.dispatch({
        ...
        payload: dataUtils.pickOwn(['title', 'url'], page)
      });
    },
